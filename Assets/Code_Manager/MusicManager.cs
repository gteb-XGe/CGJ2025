...
{
    [SerializeField] private AudioSource musicSource;  // 主背景音乐
    private Dictionary<string, AudioSource> audioSources = new Dictionary<string, AudioSource>();
    private Dictionary<string, AudioClip> audioClips = new Dictionary<string, AudioClip>();

    //音频路径文件为 public const string BGM_MAIN = "Audio/Music/bgm_main";形式
    ...
    public AudioClip GetAudio(string path)//获取音频并缓存,避免重复加载
    {
    ...
    public void PlayMainMusic(string path, float volume = 1.0f, bool loop = true)//播放主音乐
    {
    ...
    public void PlaySFX(string path, float volume = 1.0f)//playoneshot可以叠加播放,用于播放物体交互等音效
    {
        this.musicSource.PlayOneShot(GetAudio(path), volume);
    }


    public AudioSource GetAudioSource(string name, bool loop = true)// 创建并获取指定名称的AudioSource
    {
    ...
    public void PlayAudio(string name, string path, float volume = 1.0f, bool loop = true)// 播放独立控制的音频
    {
        AudioClip clip = GetAudio(path);
        if (clip == null) return;
    ...
    public void SetAudioVolume(string name, float valume = 0f)//指定音频的音量
    {
        if (audioSources.TryGetValue(name, out AudioSource source))
        {
    ...
    public void RemoveAudioSource(string name)//删除不需要的音频
    {
        if (audioSources.TryGetValue(name, out AudioSource source))
        {
            source.Stop(); // 停止播放
    ...
    // 清理所有可移除的音频源
    public void CleanupAudioSources(List<string> exceptions = null)//exceptions为需要的音频
    {
